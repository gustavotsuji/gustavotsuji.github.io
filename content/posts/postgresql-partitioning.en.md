...
```sql
-- 1. Create the partitioned master table (use IDENTITY instead of SERIAL)
CREATE TABLE events (
...
Before and after applying partitioning, validate the impact with `EXPLAIN ANALYZE` and load tests. Useful examples:

```sql
-- Measure time and buffer usage for the query (run before and after partitioning)
EXPLAIN (ANALYZE, BUFFERS, TIMING)
SELECT count(*) FROM events WHERE event_date >= '2024-01-01';

-- Repeat after partitioning and compare 'actual time' and buffer hits/reads
```

Compare key metrics: total time (actual time), number of pages read (buffers), and whether the planner pruned partitions (observe which partitions were visited). Use these readings to justify gains and tune indexes or partitioning strategy.
...
-- Example: tell pg_partman to manage the parent table (monthly partitions)
